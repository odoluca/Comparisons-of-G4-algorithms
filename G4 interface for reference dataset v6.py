...
nonG4List=[]
useG4List = (
44, 71, 95, 26, 149, 56, 162, 145, 253, 129, 171, 271, 244, 285, 277, 108, 104, 73, 53, 37, 187, 236, 66, 296, 93,
...
33, 239)
usenonG4List = (
343, 370, 314, 390, 359, 316, 344, 388, 391, 360, 339, 362, 322, 321, 324, 328, 356, 331, 352, 366, 373, 363, 369, 311,
300, 312, 307, 301, 376, 319, 330, 385, 389, 377, 336, 349, 387, 368, 306, 345, 361, 318, 340, 380, 354, 323, 332, 371,
313, 342)

for line in output.splitlines():
    if line.__contains__("not GQ_"):
        G4no=int(re.search(r"[0-9]+",line).group(0))
        if G4no not in nonG4List and G4no not in usenonG4List:
        ...
        G4no=int(re.search(r"[0-9]+",line).group(0))
        if G4no not in G4List and G4no not in useG4List:
            score=G4HScore(re.search(r"[ATCUG]{5,}",line).group(0))
            if abs(score)>=G4HScoreTreshold:
                G4List.append(G4no)
                TP+=1
...
FN=298-TP-len(useG4List)
TN=94-FP-len(usenonG4List)
